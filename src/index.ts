...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      ...
      {
        name: "get_book_marks_and_reviews",
        description: "Get all highlights and ideas and reviews for a specific book, organized by chapter",
        inputSchema: {
          type: "object",
          properties: {
            book_id: {
  ...
  try {
    ...
    switch (request.params.name) {
      ...
      case "get_bookshelf": {
        ...
        // 有笔记的书籍
        const booksWithideas = notebookBooks.length;
      ...
      // 获取指定书籍的所有划线和笔记
      case "get_book_marks_and_reviews": {
        ...
        const result: any = {
          book_id: bookId, // 书籍ID
          book_title: bookTitle, // 书名
          book_info: { // 完整的书籍信息
            author: bookInfo.author || "",
            translator: bookInfo.translator || "", // 译者
            publisher: bookInfo.publisher || "", // 出版社 
            isbn: bookInfo.isbn || "", // ISBN
            description: bookInfo.intro || "", // 简介
            cover: bookInfo.cover || "", // 封面
            format: bookInfo.format || "", // 格式
            publish_time: bookInfo.publishTime || "", // 出版时间
            word_count: bookInfo.totalWords || 0, // 总字数
            rating: bookInfo.newRating ? (bookInfo.newRating / 100) : 0, // 评分
            category: bookInfo.category || "" // 分类
          },
          reading_status: { // 阅读状态信息
            progress: readInfo.book?.progress || 0, // 阅读进度
            reading_time: readInfo.book?.readingTime || 0, // 阅读时间（秒）
            reading_time_formatted: formatReadingTime(readInfo.book?.readingTime || 0), // 阅读时间（格式化）
            start_reading_time: startReadingTimeISO, // 开始阅读时间
            has_started_reading: startReadingTime > 0, // 是否已开始阅读
            last_read_time: readInfo.book?.updateTime 
              ? new Date(readInfo.book.updateTime * 1000).toISOString() 
              : "", // 上次阅读时间
            finish_reading: bookInfo.finishReading === 1 // 是否已读完
          },
          total_highlights: highlights.length, // 总划线数
          // 过滤掉type等于4的条目，计算剩余想法数
          total_ideas: reviews.filter((review: any) => review.type !== 4).length, // 想法数,不包含点评
          total_reviews: reviews.filter((review: any) => review.type == 4).length, // 想法数,不包含点评
          last_updated: new Date().toISOString() // 数据更新时间
        };
        ...
        if (includeChapters && organizeByChapter) {
          ...
          reviews.forEach((review: any) => {
            // 确保所有必要的字段都存在
            if (review.type === 4 || !review.content) {
              return;
            }
            ...
            if (chapter) {
              chapter.ideas.push(noteData);
            } else {
              result.uncategorized.ideas.push(noteData);
            }
          });
          ...
          const cleanAndRemoveEmpty = (chapters: any[]): any[] => {
            return chapters.filter(chapter => {
              ...
              return (
                ...
                (chapter.ideas && chapter.ideas.length > 0) ||
          ...
          reviews.forEach((review: any) => {
            // 
            // 排除掉review中type为4的条目，并且检查content和chapterUid是否存在
            if (review.type === 4 || !review.content || !review.chapterUid ) return;
        ...
        // 将reviews中条目中review的type=4的放到result.reviews中
        result.reviews = reviews
          .filter((review: any) => review.type === 4)
          // createTime 是时间戳（秒），将其格式化为 ISO 字符串
          .map(({ content, createTime }) => ({ content, create_time: new Date(createTime* 1000).toISOString() }));
...
// 此代码行的作用是构建传递给命令行的参数。具体来说，会将不同来源的配置信息（如CC_URL、CC_ID等）收集到一个对象 `args` 中，
// 然后将这个对象序列化为JSON字符串，并作为命令行参数传递给后续的程序。这些参数会在 `WeReadApi` 类的 `parseCommandArgs` 方法中被解析，
// 用于后续的操作，例如获取微信读书的Cookie。这样做的目的是为程序提供灵活的配置方式，允许用户通过命令行或者环境变量来指定关键信息。
